...
import com.donaciones.donacionesbackend.util.PasswordService;
...
public class AuthUnificadoController {
        ...
        try {
            ...
            switch (rol.toUpperCase()) {
                case "DONANTE":
                    ...
                    if (donante.isPresent() && passwordService.matches(password, donante.get().getPassword())) {
                        Donante donanteLogueado = donante.get();
                        if (passwordService.needsUpgrade(donanteLogueado.getPassword())) {
                            donanteLogueado.setPassword(passwordService.encode(password));
                            donanteRepository.save(donanteLogueado);
                        }
                        response.put("success", true);
                        response.put("usuario", donanteLogueado);
                ...
                case "ADMINISTRADOR":
                    ...
                    if (admin.isPresent() && passwordService.matches(password, admin.get().getPassword())) {
                        Administrador adminLogueado = admin.get();
                        if (passwordService.needsUpgrade(adminLogueado.getPassword())) {
                            adminLogueado.setPassword(passwordService.encode(password));
                            administradorRepository.save(adminLogueado);
                        }
                        response.put("success", true);
                        response.put("usuario", adminLogueado);
                ...
                case "ORGANIZACION":
                    ...
                    if (organizacion.isPresent() && passwordService.matches(password, organizacion.get().getPassword())) {
                        Organizacion orgLogueada = organizacion.get();
                        if (passwordService.needsUpgrade(orgLogueada.getPassword())) {
                            orgLogueada.setPassword(passwordService.encode(password));
                            organizacionRepository.save(orgLogueada);
                        }
                        response.put("success", true);
                        response.put("usuario", orgLogueada);
    ...
    // Registro de usuarios nuevos según el rol elegido
    ...
    public ResponseEntity<?> register(@RequestBody Map<String, Object> registerData) {
        try {
            ...
            if ("DONANTE".equals(rol)) {
                ...
                donante.setPassword(passwordService.encode(password));
                ...
                admin.setApellido((String) registerData.get("apellido")); // Los admin tambien tienen apellido
                ...
                organizacion.setPassword(passwordService.encode(password));
